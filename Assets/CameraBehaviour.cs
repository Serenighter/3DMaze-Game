...
// klasa CameraBehaviour dziedziczy od klasy MonoBehaviour
public class CameraBehaviour : MonoBehaviour {

	
	// Tablica przechowująca obiekty cube potrzebne do generacji labiryntu
	GameObject[] CubeTab = new GameObject[300]; 
	//Stos 'walls' niezbędny do zarządzania generowanymi ściankami labiryntu podczas jego generacji 
	Stack<MazeRect> walls = new Stack<MazeRect>();
	// Zmienne stosowane do śledzenia punktów, życia i stanu gry
	static int score = 0;
	static int live = 100;
	...
	public static int CapsuleCount { get { return capsulecount; } set { capsulecount = value; } }
	// Materials, obiekty, meshe użyte w grze
	public Material floorMaterial;
	public Transform capsule;
	public Transform PlayerBall;
	GameObject player;
	
	// Metoda pozwalająca na usuwanie obiektów w grze ze sceny
	void DestroyObjects()
	{
	...
	// Redukuje życie gracza w zależności od konkretnej sytuacji w grze	
	...
	// Metoda zajmująca się przejściem do następnego poziomu gry wywoływana po spełnieniu konkretnych warunków
	...
	// Inicjalizacja całego labiryntu w ramach konkretnej prostokątnej granicy (MazeRect)
	void initMaze(MazeRect r)
	{
		int wl = 0;// liczba ścian
		// Tworzenie podłogi labiryntu
		var f = CubeTab[wl++];
		...
		// Tworzenie granic labiryntu
		var c1 = CubeTab[wl++];
		c1.transform.position = new Vector3(r.x1,-50,(r.y2+r.y1)/2);
		c1.transform.localScale = new Vector3(2,7,r.y2-r.y1);
        c1.GetComponent<Renderer>().material.color = new Color(Random.value, Random.value, Random.value);

        var c2 = CubeTab[wl++];
		c2.transform.position = new Vector3(r.x2,-50,(r.y2+r.y1)/2);
		c2.transform.localScale = new Vector3(2,7,r.y2-r.y1);
        c2.GetComponent<Renderer>().material.color = new Color(Random.value, Random.value, Random.value);

        var c3 = CubeTab[wl++];
		c3.transform.position = new Vector3((r.x2 + r.x1)/2,-50,r.y1);
		c3.transform.localScale = new Vector3(r.x2-r.x1,7,2);
        c3.GetComponent<Renderer>().material.color = new Color(Random.value, Random.value, Random.value);

        var c4 = CubeTab[wl++];
		c4.transform.position = new Vector3((r.x2 + r.x1)/2,-50,r.y2);
		c4.transform.localScale = new Vector3(r.x2-r.x1,7,2);
        c4.GetComponent<Renderer>().material.color = new Color(Random.value, Random.value, Random.value);
        // Inicjalizacja stosu potrzebnego do generacji labiryntu, ścian
        int c = 1;
		walls.Clear();
		walls.Push (r);
		while (walls.Count > 0)
		{
			MazeRect w = walls.Pop(); // Wzięcie prostokątu ze stosu
			// Pominięcie zbyt małych prostokątów, które nie mogą zostać już podzielone
			if ((w.x2 - w.x1 <= 10) || (w.y2 - w.y1 <= 10)) continue;
			// 	Wybieranie wertykalnych lub horyzontalnych ścian	 
			c = c + 1;
			if (c > 2) c = 1;
			if (c < 2)
			{
			   // Generacja wertykalnej ściany
			   float x = Random.Range ( 1, (int)(w.x2 - w.x1)/10 );
			   x = w.x1 + x * 10;
			   // Podział prostokąta na dwa pomniejsze
			   walls.Push ( new MazeRect(w.x1,w.y1,x, w.y2));
			   walls.Push ( new MazeRect(x,w.y1, w.x2, w.y2));
			   // Tworzenie wnęk w wertykalnej ścianie
			   int d = (int)(w.y2-w.y1-10) / 10;
			   d = (int)w.y1 + Random.Range (1,d) * 10;
				
				// Stworzenie dwóch sekcji wertykalnej ściany
			   var cube = CubeTab[wl++];
               ...
               cube2.GetComponent<Renderer>().material.color = new Color(Random.value, Random.value, Random.value);
            } 
			else
			{
				// Generacja horyzontalnej ściany
                ...
                // Tworzenie wnęk w horyzontalnej ścianie
                int d = (int)(w.x2-w.x1-10) / 10;
			    d = (int)w.x1 + Random.Range (1,d) * 10;
				
				// Stworzenie dwóch sekcji horyzontalnej ściany
			    var cube = CubeTab[wl++];
                ...
                cube2.GetComponent<Renderer>().material.color = new Color(Random.value, Random.value, Random.value);
                // Ustawienie nieużytych 'cube' na domyślną pozycję i skalę
	...
	{  		
	   ...
	   GUI.Label(new Rect(getWidth(0.5f), 1,getWidth(0.5f), getHeight(0.1f)),"Live: "+live+" %",labelstyle);
		GUI.Label(new Rect(1, 1, getWidth(1f), getHeight(1f)), "Remaining enemies: " + capsulecount, labelstyle);
	   if (!player)
	   {
